...
{
	const auto data = EnvLoader::Load(filePath);
	const auto botToken = GetRequired(data, "BOT_TOKEN");

	if (botToken.find(':') == std::string::npos)
	{
		throw std::runtime_error("Неверный BOT_TOKEN");
	}

	return Config(botToken, ParseUsers(GetRequired(data, "ALLOWED_USERS")), GetRequired(data, "BFO_BASE_URL"), GetRequired(data, "BFO_SEARCH_PATH"), GetLong(data, "BFO_PAGE_SIZE"), GetLong(data, "BFO_CONNECT_TIMEOUT_SEC"), GetLong(data, "BFO_READ_TIMEOUT_SEC"), GetLong(data, "BFO_WRITE_TIMEOUT_SEC"), GetRequired(data, "BFO_USER_AGENT"), GetRequired(data, "POLZA_BASE_URL"), GetRequired(data, "POLZA_API_KEY"), GetRequired(data, "POLZA_MODEL"));
}
...
Config::Config(
	...
	, m_polzaBaseUrl(std::move(polzaBaseUrl))
	, m_polzaApiKey(std::move(polzaApiKey))
	, m_polzaModel(std::move(polzaModel))
{
}
