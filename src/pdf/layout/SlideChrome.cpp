#include "layout/SlideChrome.hpp"

#include "layout/TextBlock.hpp"
#include "layout/TextStyles.hpp"
...
namespace
{
    ...
    void AssertIsPositivePageNumber(int pageNumber)
    {
        if (pageNumber <= 0)
        {
    ...
    void AssertIsKnownAsset(const Theme& theme, AssetRole role)
    {
    ...
    AssetRole FooterLogoRole(bool inverse)
    {
        return inverse ? AssetRole::LogoInverse : AssetRole::Logo;
    }

    Color FooterTextColor(const Theme& theme, bool inverse)
    {
