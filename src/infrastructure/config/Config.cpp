...
constexpr std::string_view CONTACTS_DB_PATH = "CONTACTS_DB_PATH";
constexpr std::string_view DEFAULT_CONTACTS_DB_PATH = "contacts.sqlite3";
...
std::string GetStringOr(const EnvLoader::EnvData& data, const std::string_view key, const std::string_view fallback)
{
	const auto it = data.find(std::string(key));
...
Config Config::LoadFromEnv(const std::filesystem::path& path)
{
	...
	config.m_contactsDbPath = GetStringOr(data, CONTACTS_DB_PATH, DEFAULT_CONTACTS_DB_PATH);
...
const std::filesystem::path& Config::GetContactsDbPath() const
{
	return m_contactsDbPath;
}
