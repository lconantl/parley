...
void AssertIsPlainValue(const std::string& value, const size_t lineNumber)
{
	...
	for (const char ch : value)
	{
		if (ch == DELIMITER || ch == COMMENT)
		{
			throw MakeError(
				lineNumber,
				std::string("Символ '") + ch + "' в значении требует кавычек, пишите KEY=\"" + value + "\"");
		}
	}
}
...
void AssertIsNotRepetitions(
	EnvLoader::EnvData& data,
...
EnvLoader::EnvData EnvLoader::Load(const std::filesystem::path& path)
{
	...
	while (std::getline(file, line))
	{
		...
		const auto compactLine = RemoveWhitespaceOutsideQuotes(line, lineNumber);
